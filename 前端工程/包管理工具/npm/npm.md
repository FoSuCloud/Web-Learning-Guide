...
* 用于检测`未使用`的或`缺失`的 npm 包依赖，可以协助发现现存代码可能存在的`幽灵依赖`，类似的还有：`npm-check` 等
...
* npm install 会`尝试更新依赖`，触发依赖结构树变化并记录到 package-lock.json 文件
* npm ci 则`严格按照 package-lock.json 内容准确安装各个依赖版本`，`在 CI/CD 环境`中能获得更强的`稳定`性，确保代码行为与开发环境尽可能一致。
