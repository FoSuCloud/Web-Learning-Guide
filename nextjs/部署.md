## 部署
* `如果nextjs被部署，那么需要添加next.config.js文件`
* 配置输出
```js
...
```
* `使用standalone 输出`
* `最后进行npm run build的时候会生成一个standalone目录`
* 下面会有当前项目的package.json,node_modules,`还会创建一个server.js文件，用于作为后端服务`
* 例如这样的
```js
...
```
* 如果我们是直接使用docker部署，那么就写一个dockerfile文件
```dockerfile
USER nextjs
