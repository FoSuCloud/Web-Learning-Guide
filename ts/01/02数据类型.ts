...
// var flag_b:boolean=new Boolean(true);//报错,Type 'Boolean' is not assignable to type 'boolean'.
 var flag_b:Boolean=new Boolean(true);//[Boolean: true]
 console.log(flag_b)

// 直接调用Boolean()获得的是类型转换为布尔的值，不是布尔包装对象！
 var flag_c:boolean=Boolean(2);
...
// a=true;//rror TS2011: Cannot convert 'boolean' to 'number'.
 ...
 var arr:string[]=['css','html','js']
 console.log(arr);//['css','html','js']
 /* 4.2 第二种形式(数组泛型) var/let/const arr:Array<type>=[] */
 let a_arr:Array<number>=[3,2,1]
 let {log}=console
 log(a_arr);//[3,2,1]

 /* 5.元组类型
  元组类型属于数组类型中的一种，不同之处在于元组可以指定多种数据类型*/
 var tunp:[string,number,boolean]=['字符串',33,true]
 log(tunp);//["字符串", 33, true]

 /* 6.枚举类型。用于标注状态 */
 enum myflag {success=1,error=0}
 var yes:myflag=myflag.success
 var no:myflag=myflag.error
 log(yes,no);//1 0
...
// 6.1 如果不设置枚举类型的值，那么默认返回的是索引，以0开始
 ...
 log(color.red);//0
 log(color.green);//1
 var color_b:color=color.blue;
 log(color_b);//2
// 6.2 只给部分枚举类型的元素设置值
 enum other {one,two=5,six}
 log(other.one);//0
 ...
 add(3,2);//5
 /* 有返回值时*/
 function del(a,b):number{
  return a-b;
 }
 log(del(30,1));//29
 // void类型如果用于变量中，那么只能被赋值为undefined/null
 var void_a:void=undefined
 var void_b:void=null;
 console.log("void:")
...
// undefined/null其实所有所有类型的子类型，就是说你可以把 null和undefined赋值给其他类型的变量。
 var num_n:number=undefined;
 var num_nn:number=null;
 var str_n:string=undefined;
 var str_nn:string=null;
...
// any类型和unkown类型的区别
 /**
  * any类型和unknwon类型都可以被任何类型赋值。
  * 但是unknown类型不可以赋值给一个非unknown/any的变量
  * */
 var unkown1:unknown = 4;
 // var num_111:number = unkown1; // Type 'unknown' is not assignable to type 'number'.
 var any_11=unkown1
 console.log(any_11)
}
fn1()
