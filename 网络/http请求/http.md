...
`例如前端获取static/video中存储的mp4文件，可以对文件进行分段获取，浏览器会自动给后端发送range请求头告诉浏览器读取的文件范围，如Range: bytes=0-`
---
...
* 502：网关无响应，没有资源响应，`可能是由于一下子有大量请求进入，nginx代理的对应服务器无法处理完成，导致无响应。一般重启该服务就可以了`
...
5. `HEAD:只请求响应文件的头部信息，不会响应文件内容！可以视为弱化版的get请求`
6. `OPTIONS请求(一般用在CORS中)是浏览器知道某个POST请求或者其他请求有可能对服务器数据产生影响时发起的预检请求，如果服务器允许请求才会继续那个请求`
* 一般请求头会包括： Access-Control-Request-Method、Access-Control-Request-Headers
* 一旦服务器通过了 预检请求，以后每次浏览器正常的 CORS 请求，就都跟简单请求一样，会有一个 Origin 头信息字段。
